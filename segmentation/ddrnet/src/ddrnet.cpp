...
ICudaEngine* createEngine(std::string wts_filename, unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights(wts_filename);
...
void APIToModel(std::string wts_filename, unsigned int maxBatchSize, IHostMemory** modelStream) {
    ...
    ICudaEngine* engine = createEngine(wts_filename, maxBatchSize, builder, config, DataType::kFLOAT);
...
int ddr_main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        APIToModel("DDRNet.wts", 1, &modelStream);
