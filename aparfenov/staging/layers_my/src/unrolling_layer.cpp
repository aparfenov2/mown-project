...
{
UnrollingLayer::UnrollingLayer() : private_nh_{ "~" }
{
  prev_robot_pose_x = 1e30;
  prev_robot_pose_y = 1e30;
}

void UnrollingLayer::onInitialize()
{
  config_ = UnrollingLayerConfig( private_nh_, name_ );
  matchSize();
  initTranslator();
  initPubSub();
...
{
    throw std::logic_error("UnrollingLayer: partial updates are not supported");
...
// std::cout << "DBG: map_idx_x " << map_idx_x << " map_idx_y " << map_idx_y
// << " start_idx_x " << start_idx_x << " start_idx_y " << start_idx_y
// << " end_idx_x " << end_idx_x << " end_idx_y " << end_idx_y
...
void dumpMap(std::string fn, const unsigned char *map, int width, int height)
{
  std::fstream of(fn, std::ios::out | std::ios::trunc);
  if (of.is_open())
  {
    of << height << " ";
    of << width << " " << std::endl;
    for (int i = 0; i < height; ++i)
    {
      ...
      {
        of << std::setw(4) << (unsigned int)map[i * width + j] << " ";
      }
      of << std::endl;
    }
    of.close();
  } else {
    ROS_FATAL_STREAM("failed to open file " << fn);
  }
}
...
void UnrollingLayer::updateMap(const std::vector<int8_t>& map, const UpdateMapMetadata& metadata)
{
  ...
  for (size_t y = metadata.start_idx_y; y < metadata.end_idx_y; y++)
  {
    ...
    // costmap_[our_index] = 10;
    }
  }
//   dumpMap("costmap.txt", costmap_, getSizeInCellsX(), getSizeInCellsY());
}
...
{
//   ROS_INFO_STREAM("updateBounds " << min_map_x_ << " " << min_map_y_ << " " << max_map_x_ << " " << max_map_y_);
  ...
  *max_y = std::max(max_wy, *max_y);
  ...
  if (prev_robot_pose_x < 1e30)
  {
    min_wx += (prev_robot_pose_x - robot_x) * 2;
    min_wy += (prev_robot_pose_y - robot_y) * 2;
    max_wx += (prev_robot_pose_x - robot_x) * 2;
    max_wy += (prev_robot_pose_y - robot_y) * 2;
  ...
  prev_robot_pose_x = robot_x;
  prev_robot_pose_y = robot_y;

  min_map_x_ = std::numeric_limits<size_t>::max();
  min_map_y_ = std::numeric_limits<size_t>::max();
  max_map_x_ = 0;
  max_map_y_ = 0;
//   ROS_INFO_STREAM("updateBounds " << *min_x << " " << *min_y << " " << *max_x << " " << *max_y);
...
//   ROS_INFO_STREAM("updateCosts " << size_x_ << " " << size_y_ << " " << min_i << " " << min_j << " " << max_i << " " << max_j);
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      if (line_array[it] > costmap_2d::FREE_SPACE && line_array[it] != costmap_2d::NO_INFORMATION) {
        master_array[it] = line_array[it];
      }
    }
  }
//   dumpMap("master_array.txt", master_array, master_grid.getSizeInCellsX(), master_grid.getSizeInCellsY());
}
