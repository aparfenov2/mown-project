...
class ODriveInterfaceAPI(object):
    ...
    def connect(self, port=None, right_axis=0, timeout=1):
        if self.driver:
            self.logger.info("Already connected. Disconnecting and reconnecting.")
        try:
            self.logger.info("Try to connect to ODRIVE.")
            self.driver = odrive.find_any(timeout=timeout)
            self.axes = (self.driver.axis0, self.driver.axis1)
            self.logger.info("ODRIVE is connected.")
        except:
    ...
    def left_temperature(self):   return 0.
    def right_temperature(self):  return 0.
