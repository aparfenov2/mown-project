...
bool FullCoveragePathPlanner::parseGrid(costmap_2d::Costmap2D *costmap_,
                                        std::vector<std::vector<bool> >& grid,
                                        float robotRadius,
                                        float toolRadius,
                                        geometry_msgs::PoseStamped const& realStart,
                                        Point_t& scaledStart)
{
  ...
  // Save map origin and scaling
  tile_size_ = nodeSize * resolution;  // Size of a tile in meters

  double wx, wy;
  costmap_->mapToWorld(0, 0, wx, wy);

  grid_origin_.x = wx - resolution / 2;  // x-origin in meters
  grid_origin_.y = wy - resolution / 2;  // y-origin in meters

  // Scale starting point
  scaledStart.x = static_cast<unsigned int>(clamp((realStart.pose.position.x - grid_origin_.x) / tile_size_, 0.0,
                             floor(costmap_->getSizeInCellsX() / tile_size_)));
  scaledStart.y = static_cast<unsigned int>(clamp((realStart.pose.position.y - grid_origin_.y) / tile_size_, 0.0,
