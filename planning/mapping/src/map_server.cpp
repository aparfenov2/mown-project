...
class MappingServer {
public:
    MappingServer(ros::NodeHandle& nodeHandle) : nh(nodeHandle), map_(std::vector<std::string>({"occupancy"})) {
        // Parameters
        nh.getParam("fixed_frame_id", FIXED_FRAME_ID);
        nh.getParam("resolution", RESOLUTION);
        nh.getParam("max_range", MAXIMUM_RANGE);
        nh.getParam("hit_logprob", HIT_LOGPROB);
        nh.getParam("miss_logprob", MISS_LOGPROB);
        nh.getParam("max_logprob", MAX_LOGPROB);
        nh.getParam("min_logprob", MIN_LOGPROB);
        ...
        nh.getParam("/planner/topics/lidar", lidar_topic_name);
        nh.getParam("/planner/topics/costmap", ocupancy_grid_topic_name);
...
int main(int argc, char** argv)
{
    ...
    MappingServer mapping_server(nh);
